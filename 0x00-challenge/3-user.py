...
class User:
    ...
    def __init__(self):
        ...
        """
        self.id = str(uuid.uuid4())  # Assign a unique UUID to the user
        self.__password = None  # Initialize password to None initially
    ...
    def password(self, pwd):
        ...
        """
        if pwd is None or not isinstance(pwd, str):  # Check if pwd is None or not a string
            self.__password = None  # Set password to None
        else:
            self.__password = hashlib.md5(pwd.encode()).hexdigest()  # Hash password using MD5
    ...
    def is_valid_password(self, pwd):
        ...
        """
        if pwd is None or not isinstance(pwd, str) or self.__password is None:  # Check if pwd is None, not a string, or __password is None
            return False
        return hashlib.md5(pwd.encode()).hexdigest() == self.__password  # Compare hashed passwords
